...
func (s *Service) StartZombieHordeRaid(c Coordinates) {
	...
	go func() {
		...
		s.rcon.Say("A zombie horde is attacking the base!")
	...
	go func() {
		...
		for {
			...
			for _, player := range players {
				...
				s.rcon.SendMessage(player.Name, "For your troubles, here's a diamond. Don't spend it all in one place!")
	...
	go func() {
		for {
			...
			for playerName, player := range players {
				s.rcon.SendMessage(playerName, "A lightning storm is approaching!")
	...
	go func() {
		...
		for {
			...
			for _, player := range players {
				...
				s.rcon.SendMessage(player.Name, "You have been selected to receive free minecart equipment! I suggest you built a crate, soon!!!")
